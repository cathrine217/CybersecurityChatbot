...
namespace CybersecurityChatbotWPF
{
    public class ChatbotEngine
    {
        ...
        private void InitializeResponses()
        {
            ...
            passwordResponses.Add("Use strong passwords with 12+ characters mixing uppercase, lowercase, numbers, and symbols!");
            passwordResponses.Add("Never reuse passwords across multiple accounts - use a password manager!");
            passwordResponses.Add("Avoid using personal info like birthdays or pet names in passwords.");
            passwordResponses.Add("Enable Two-Factor Authentication whenever possible for extra security!");
            responseDatabase.Add("password", passwordResponses);

            // Phishing responses
            List<string> phishingResponses = new List<string>();
            phishingResponses.Add("Always check email sender addresses carefully - scammers use lookalike domains!");
            phishingResponses.Add("Never click links in suspicious emails. Hover to see the actual URL first!");
            phishingResponses.Add("Look for spelling errors and urgent language - common phishing tactics!");
            phishingResponses.Add("Legitimate companies never ask for passwords via email!");
            responseDatabase.Add("phishing", phishingResponses);

            // Privacy responses
            List<string> privacyResponses = new List<string>();
            privacyResponses.Add("Review your social media privacy settings regularly!");
            privacyResponses.Add("Limit what personal info you share publicly online.");
            privacyResponses.Add("Use privacy-focused browsers and search engines.");
            privacyResponses.Add("Check app permissions regularly!");
            responseDatabase.Add("privacy", privacyResponses);

            // Malware responses
            List<string> malwareResponses = new List<string>();
            malwareResponses.Add("Keep your antivirus software updated and run regular scans!");
            malwareResponses.Add("Don't download software from untrusted websites.");
            malwareResponses.Add("Be careful with USB drives from unknown sources.");
            malwareResponses.Add("Enable automatic updates for your operating system!");
            responseDatabase.Add("malware", malwareResponses);

            // Safe Browsing responses
            List<string> safeBrowsingResponses = new List<string>();
            safeBrowsingResponses.Add("Look for HTTPS and the padlock icon before entering sensitive data!");
            safeBrowsingResponses.Add("Avoid using public Wi-Fi for banking or sensitive transactions.");
            safeBrowsingResponses.Add("Use a VPN when connecting to public networks.");
            safeBrowsingResponses.Add("Clear your browser cookies and cache regularly.");
            ...
            generalResponses.Add("Always backup your important data regularly!");
            generalResponses.Add("Keep all your software updated for security patches.");
            generalResponses.Add("Be careful what you share on social media!");
            generalResponses.Add("Use different passwords for different accounts.");
        ...
        public string GetTopicResponse(string topic, string userName)
        {
            if (responseDatabase.ContainsKey(topic))
            {
                ...
                return responses[randomIndex] + "\n\nWould you like more tips on this topic?";
            }
            ...
            Dictionary<string, string> memory, string userName)
        {
            ...
            if (ContainsAny(lowerInput, new string[] { "another tip", "tell me more", "more" }))
            {
            ...
            if (memory.ContainsKey("name") && ContainsAny(lowerInput, new string[] { "my name", "remember me" }))
            {
                return "I remember! You said your name is " + memory["name"] + "!";
            }

            // Check for memory - interest recall
            if (memory.ContainsKey("interest") && ContainsAny(lowerInput, new string[] { "remember", "interested" }))
            {
                return "As someone interested in " + memory["interest"] + ", you might want to learn more about that!";
            }

            // Check for menu/help
            if (ContainsAny(lowerInput, new string[] { "menu", "help", "options" }))
            {
                return GetHelpMessage();
            }

            // Greeting response
            if (ContainsAny(lowerInput, new string[] { "hello", "hi", "hey", "greetings" }))
            {
                return "Hello " + userName + "! Type 'menu' to see all topics I can help you with!";
            }

            // Thank you response
            if (ContainsAny(lowerInput, new string[] { "thank", "thanks" }))
            {
                return "You're welcome, " + userName + "! Stay safe online!";
            }

            // Keyword matching
            foreach (KeyValuePair<string, string> kvp in keywordMap)
            {
                if (lowerInput.Contains(kvp.Key))
                {
                    ...
                    int randomIndex = random.Next(responses.Count);
                    return AdjustResponseForSentiment(responses[randomIndex], sentiment, userName);
                }
            }
